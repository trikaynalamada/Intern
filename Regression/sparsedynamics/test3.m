...
x0=[1.9;4;2];  % Initial condition

% Integrate
tspan=[0:.01:20];
N = length(tspan);
options = odeset('RelTol',1e-12,'AbsTol',1e-12*ones(1,n));
[t,x]=ode45(@(t,x) paper2(t,x),tspan,x0,options);
...
lambda = .05;      % lambda is our sparsification knob.
Xi = sparsifyDynamics(Theta,dx,lambda,n)
%poolDataLIST({'x','y','z'},Xi,n,polyorder,usesine);

%% FIGURE 1:  LORENZ for T\in[0,20]
tspan = [0 20];
[tA,xA]=ode45(@(t,x)paper2(t,x),tspan,x0,options);   % true model
