...
n = 2;          % 2D system
A = [-1 2; -2 -1];  % dynamics
rhs = @(x)A*(((x)./(1+x.^4)));   % ODE right hand side
tspan=[0:.01:25];   % time span
x0 = [2 ; 4];        % initial conditions
options = odeset('RelTol',1e-10,'AbsTol',1e-10*ones(1,n));
[t,x]=ode45(@(t,x)paper3(t,x),tspan,x0,options);  % integrate

%% compute Derivative 
eps = .01;      % noise strength
for i=1:length(x)
    dx(i,:) = paper3(0,x(i,:));
end
...
[tA,xA]=ode45(@(t,x)paper3(t,x),tspan,x0,options);   % true model
